...
Router.run(routes, Router.HistoryLocation, function(Application, state) {
  ...
  // This is not needed when the server notifies the client about changes
  // (WebSocket, SSE)
  if(!initialRun) {
  ...
  // when the data is not fully fetched after the timeout components
  // are rendered (with missing/old data)
  withTimeout(async.forEach.bind(
    async,
    state.routes,
